...
const BlogsPage = ({ posts = blogPostsArray }) => {
  ...
  // Create a reference to the top of the blogs section
  ...
  const handleTogglePosts = () => {
    if (isAllShown) {
      // Close articles and scroll back up smoothly
      setVisibleCount(INITIAL_COUNT);
      
      // Scroll back to the top of the section with a slight delay to allow animations to start
      setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Advice Section Title - Ref added here to act as the scroll anchor */}
      <section ref={blogsSectionRef} className="py-15 px-8 md:px-16 lg:px-20 border-t border-gray-100 pt-30">
        <div className="max-w-8xl mx-10 text-center md:text-left">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="heading-2 font-bold text-[#2D2D2D] mb-8 tracking-wide uppercase">
      ...
      <section className="pb-24 px-8 md:px-16 lg:px-20">
        <div className="max-w-8xl mx-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 space-y-5 md:space-y-0">
            {/* Wrapped in AnimatePresence for smooth exit animations */}
            <AnimatePresence mode="popLayout">
              {displayedPosts.map((post, idx) => {
                // Calculate delay so newly loaded items stagger quickly, but don't delay forever
                const delayIndex = idx >= INITIAL_COUNT ? idx - INITIAL_COUNT : idx;
                const transitionDelay = Math.min(delayIndex * 0.1, 0.4);

                return (
                  <motion.div 
                    key={post.id} 
                    layout // Ensures smooth reflow of the grid
                    initial={{ opacity: 0, y: 30, scale: 0.98 }} 
                    whileInView={{ opacity: 1, y: 0, scale: 1 }} 
                    exit={{ opacity: 0, y: -20, scale: 0.95, transition: { duration: 0.3 } }}
                    viewport={{ once: true, margin: "-50px" }} 
                    transition={{ duration: 0.5, delay: transitionDelay, ease: "easeOut" }} 
                  ...
                  >
                    <div className="aspect-[16/10] rounded-[2.5rem] overflow-hidden mb-6 relative shadow-xl shadow-gray-200/50 border border-gray-100">
                      <img src={post.image} alt={post.title} className="w-full h-full object-cover transition-transform duration-1000 scale-110 group-hover:scale-120" />
